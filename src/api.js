...
const external = (() => {
  async function getSearchC(country) {
    try {
      ...
      dom.createWeatherInfoDivC(parseData.feels_like, parseData.humidity, parseData.pressure, parseData.temp, icon, city, parseData.temp_max, parseData.temp_min);     
    }catch (error) {
  ...
  async function getSearchF(country) {
    try {
      ...
      let icon = weatherData.weather[0].icon;
      let city = weatherData.name;
      dom.createWeatherInfoDivF(parseData.feels_like, parseData.humidity, parseData.pressure, parseData.temp, icon, city, parseData.temp_max, parseData.temp_min);     
    }catch (error) {
      alert("Please Enter the Valid name of a City");
    }
